...
@add_variable_metadata(variable_name="beta_band_rEEG", units="μV", window_size=2, slide_size=2, subfunctions=["mean", "median", "STDEV", "coefficient_of_variance", "5th_percentile", "95th_percentile", "width", "mann_kendall_tau", "theil_sen_slope", "theil_sen_y_int", "lin_reg", "lin_reg_y"])
def rEEG_beta_band(data,raw_obj):
    data_point = np.percentile(data, 95) - np.percentile(data, 5)
    return data_point

@add_variable_metadata(variable_name = "beta_rEEG_prop", units = "", window_size = 60, slide_size = 30, subfunctions = {"_0_10": default_stats, "_10_25": default_stats, "_25_50": default_stats, "_50_100": default_stats, "_over_100": default_stats})
def beta_rEEG_props(data,raw_obj):
    rEEG_window = []
...
@add_variable_metadata(variable_name="higuchi_fractal_dimension_beta", units="units", window_size=60, slide_size=30, subfunctions=default_stats)
def higuchi_fractal_dimension_beta(data, raw_obj):
    hfd_value, _ = nk.fractal_higuchi(data, k_max=10)
    return hfd_value

#3: Envelope
@add_variable_metadata(variable_name="beta_envelope", units="μV²", window_size=60, slide_size=30, subfunctions={"beta_env_mean": default_stats, "beta_env_STDEV": default_stats})
def beta_envelope(data, raw_obj):
    env = np.abs(signal.hilbert(data)) ** 2
    beta_env_mean = np.nanmean(env)
    beta_env_STDEV = np.nanstd(env, ddof=1)
    return beta_env_mean, beta_env_STDEV

#4: Kurtosis
@add_variable_metadata(variable_name="beta_kurtosis", units="units", window_size=60, slide_size=30, subfunctions=default_stats)
def beta_kurtosis(data, raw_obj):
    return kurtosis(data, fisher=False, nan_policy='omit')

#5: Power
@add_variable_metadata(variable_name="beta_power", units="μV²", window_size=60, slide_size=30, subfunctions={"mean_power_beta": default_stats, "stdev_beta": default_stats})
def beta_power(data, raw_obj):
    mean_power_beta =  np.nanmean(np.abs(data) ** 2)
    stdev_beta = np.std(data, ddof=1)
    return mean_power_beta, stdev_beta

#6: Skew
@add_variable_metadata(variable_name="beta_skew", units="units", window_size=60, slide_size=30, subfunctions=default_stats)
def beta_skew(data, raw_obj):
