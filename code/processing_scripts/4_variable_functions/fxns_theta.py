...
@add_variable_metadata(variable_name="theta_band_rEEG", units="μV", window_size=2, slide_size=2, subfunctions=["mean", "median", "STDEV", "coefficient_of_variance", "5th_percentile", "95th_percentile", "width", "mann_kendall_tau", "theil_sen_slope", "theil_sen_y_int", "lin_reg", "lin_reg_y"])
def rEEG_theta_band(data,raw_obj):
    data_point = np.percentile(data, 95) - np.percentile(data, 5)
    return data_point

@add_variable_metadata(variable_name = "theta_rEEG_prop", units = "", window_size = 60, slide_size = 30, subfunctions = {"_0_10": default_stats, "_10_25": default_stats, "_25_50": default_stats, "_50_100": default_stats, "_over_100": default_stats})
def theta_rEEG_props(data,raw_obj):
    rEEG_window = []
...
@add_variable_metadata(variable_name="higuchi_fractal_dimension_theta", units="units", window_size=60, slide_size=30, subfunctions=default_stats)
def higuchi_fractal_dimension_theta(data, raw_obj):
    hfd_value, _ = nk.fractal_higuchi(data, k_max=10)
    return hfd_value

#3: Envelope
@add_variable_metadata(variable_name="theta_envelope", units="μV²", window_size=60, slide_size=30, subfunctions={"theta_env_mean": default_stats, "theta_env_STDEV": default_stats})
def theta_envelope(data, raw_obj):
    env = np.abs(signal.hilbert(data)) ** 2
    theta_env_mean = np.nanmean(env)
    theta_env_STDEV = np.nanstd(env, ddof=1)
    return theta_env_mean, theta_env_STDEV

#4: Kurtosis
@add_variable_metadata(variable_name="theta_kurtosis", units="units", window_size=60, slide_size=30, subfunctions=default_stats)
def theta_kurtosis(data, raw_obj):
    return kurtosis(data, fisher=False, nan_policy='omit')

#5: Power
@add_variable_metadata(variable_name="theta_power", units="μV²", window_size=60, slide_size=30, subfunctions={"mean_power_theta": default_stats, "stdev_theta": default_stats})
def theta_power(data, raw_obj):
    mean_power_theta =  np.nanmean(np.abs(data) ** 2)
    stdev_theta = np.std(data, ddof=1)
    return mean_power_theta, stdev_theta

#6: Skew
@add_variable_metadata(variable_name="theta_skew", units="units", window_size=60, slide_size=30, subfunctions=default_stats)
def theta_skew(data, raw_obj):
