# Author:       Brian Murphy, edited by Alex Gimeno
...
def get_band_power(x, Fs, freq_band, window_size, relative=False):
    """
    Get power for a single frequency band
    
    Inputs:
    - x (arr of float): signal data
    - Fs (int): Sampling frequency (Hz)
    - freq_band (tuple of int or float): the frequency band whose power we want to analyze (e.g., (0.5-3) [NB: this is a tuple])
    - window_size (int): size of the window to be interpreted in SECONDS
    - relative (bool): If True, return relative power (band power normalized by total signal power, vs raw power value)
    
    Outputs:
    - power (float): power value (μV²/Hz for raw power, unitless [between 0 and 1] for relative [power]) for the frequency band in the data given
    """
    params_st = {"freq_bands": [freq_band], "method": "periodogram", "L_window":window_size, "window_type":"hamm", "overlap":0}
    
    pxx, itotal_bandpass, f_scale, N, fp = gen_spectrum(x, Fs, params_st, 1) #Each entry in pxx is a real number representing the power of a given frequency (amplitude squared of its given sin wave). The complex number representing the sin wave is built in gen_spectrum (with the FFT function), but the power is then calculated before gen_spectrum gives its final output. itotal_bandpass is our array of pxx indices corresponding to all frequencies in our band of interest (since pxx will contain frequencies from 0 [or DC signal portion] to the nyquist frequnecy [half the sampling rate])
    pxx = pxx * Fs
    ...
    # Calculate indices for the frequency band
    istart = np.ceil(freq_band[0] * f_scale).astype(int) - 1
    istop = np.floor(freq_band[1] * f_scale).astype(int)
    
    ibandpass = np.array(range(istart, istop)) + 1
    ibandpass = np.clip(ibandpass, 0, len(pxx) - 1)
    
    power = np.nansum(pxx[ibandpass] / (N * pxx_total))
    
    return power

# Usage example:
# alpha_power = get_band_power(signal, 256, (8, 13), relative=False)
...
def spectral_hfd(x, Fs, feat_name, params_st):
    pxx, dum, f_scale, dum, dum = gen_spectrum(x, Fs, params_st)
    ...
    for p in range(N_freq_bands):
        ...
        featx[p], _ = nk.fractal_higuchi(pr, k_max=10)
